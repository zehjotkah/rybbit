...
/**
 * Returns null when none of the site goals produces a valid conversion
 * condition — the handler responds with empty data in that case.
 */
export const buildGoalTimeSeriesQuery = (
  query: GetGoalTimeSeriesRequest["Querystring"],
  siteId: number,
...
export const getGoalTimeSeries = analyticsRoute<GetGoalTimeSeriesRequest>(
  "goal time series",
  async (req: FastifyRequest<GetGoalTimeSeriesRequest>, res: FastifyReply) => {
    ...
    const query = buildGoalTimeSeriesQuery(req.query, siteId, siteGoals);
    if (query === null) {
      return res.send({ data: [] });
    }

    const data = await runAnalyticsQuery<GoalTimeSeriesPoint>({
      query,
      params: {
        siteId,
        timeZone,
