...
/**
 * The two-step filter builder (pick a dimension, then an operator and value)
 * that the Filter popover has always shown. It is a controlled fragment so
 * the same picker can add filters to the dashboard and to a segment being
 * edited in a dialog. `pendingRef` lets the host commit a half-built filter
 * when its popover is dismissed, matching the previous behaviour.
 */
export function FilterPicker({
  ...
  setParameter: (parameter: FilterParameter | null) => void;
}) {
  const t = useExtracted();
  const getParameterLabel = useParameterLabel();
  ...
  return (
    <Command>
      <div className="px-3 pt-3 pb-1 text-xs text-neutral-500 dark:text-neutral-400">{t("Filter stats by")}</div>
        ...
        <CommandGroup>
          {parameterOptions.map(option => (
            <CommandItem
              ...
              onSelect={() => setParameter(option.value)}
              className="cursor-pointer"
            >
