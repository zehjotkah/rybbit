...
export function EventProperties({ properties, isLoading, selectedEvent, size = "small" }: EventPropertiesProps) {
  ...
  const sortedKeys = Object.keys(groupedProperties).sort((a, b) => {
    const sumA = groupedProperties[a].reduce((sum, prop) => sum + prop.count, 0);
    const sumB = groupedProperties[b].reduce((sum, prop) => sum + prop.count, 0);
    return sumB - sumA;
  });
      ...
      {sortedKeys.map(key => {
        ...
        return (
          <div key={key} className={cn("flex flex-col gap-1", size === "small" ? "text-xs" : "text-sm")}>
            {/* Property Key Header */}
            <div className="font-semibold  text-primary py-1 border-b border-neutral-800">{key}</div>

            {/* Property Values */}
            <div className="pl-4 flex flex-col gap-2">
              {values.map(property => {
                const totalCount = properties
                  .filter(event => event.propertyKey === property.propertyKey)
                      ...
                      <div className="truncate max-w-[70%]">
                        <Tooltip>
                          <TooltipTrigger asChild>
                            <div className="flex items-center">{property.propertyValue}</div>
                          </TooltipTrigger>
                          <TooltipContent>
                            <p className="max-w-7xl">{property.propertyValue}</p>
                          </TooltipContent>
                        </Tooltip>
                      </div>
        ...
        <div key={groupIndex} className={cn("flex flex-col gap-1", size === "small" ? "text-xs" : "text-sm")}>
          ...
          <div className="pl-4 flex flex-col gap-2">
            {Array.from({ length: propertyCounts[groupIndex] }).map((_, propIndex) => {
              ...
              const barWidths = generateBarWidths(propertyCounts[groupIndex]);
              ...
              return (
                ...
                >
                  ...
                  <div className="z-10 flex justify-between items-center w-full">
                    ...
                    <div className="flex gap-2">
                      <div className="h-4 bg-neutral-800 rounded animate-pulse" style={{ width: "40px" }}></div>
                    </div>
                  </div>
                </div>
              );
