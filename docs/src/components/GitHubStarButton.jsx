    ...
    const fetchStarCount = async () => {
      try {
        const response = await fetch(
          "https://api.github.com/repos/rybbit-io/rybbit"
        );
  ...
  return (
    <div className="mb-6 md:mb-8">
      ...
      >
        ...
        <span>Star us!</span>
        <div className="flex items-center gap-1 ml-1 mr-0.5  py-0.5 bg-neutral-700/50 rounded-full text-xs">
          <Star className="w-3 h-3 text-yellow-400 fill-current" />
          {isLoading ? (
            <div className="w-7 h-3.5 bg-neutral-600 rounded-sm animate-pulse"></div>
          ) : (
            starCount && <span className="h-3.5 w-7">{starCount}</span>
          )}
        </div>
      </a>
    </div>
