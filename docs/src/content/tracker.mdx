...
Set to `"false"` to disable automatic pageview tracking on initial load and History API changes. You will need to manually call `window.rybbit.pageview()` or `window.rybbit.track()`.

-   **Default:** `"true"`
-   **Example:** `data-auto-track="false"`
