...
<Callout type="warn">
**Critical**: The `X-Forwarded-For` header must contain the original client IP for accurate geolocation and user identification, and the **First-Party Proxy** toggle must be enabled in your site settings. Don't rely on `X-Real-IP` — on Rybbit Cloud it never reaches the backend (our edge network consumes it), and AWS CloudFront cannot send it at all.
</Callout>
...
### Proxied Requests Are Detected as Bots

**Symptoms:**
- `/analytics/track` returns `200 OK`
- Events do not appear in the normal analytics dashboard (only under bot traffic)
- Tracking works fine when the script talks to Rybbit directly, without the proxy

There are two independent causes, and server-side proxies often hit **both**. Work through them in order.

#### Cause 1: Your proxy strips the browser's headers

Rybbit's bot detection scores requests on headers that every real browser sends: `Accept`, `Accept-Language`, `Accept-Encoding` (with `gzip`), and the `Sec-Fetch-*` headers that all Chromium browsers attach. A proxy that forwards only `User-Agent` and the visitor IP produces requests indistinguishable from a script — they are accepted with `200 OK` but classified as bots.

**Fix:** forward **all** incoming request headers, minus `Host` and hop-by-hop headers (`Connection`, `Content-Length`). Hand-picking headers is the most common mistake in PHP/WordPress-style proxies.

```php
// PHP example — forward everything except Host/hop-by-hop
$headers = [];
...
#### Cause 2: Rybbit sees your proxy's IP instead of the visitor's

First-party proxies typically run on hosting/datacenter infrastructure, so if the visitor IP is not forwarded (or not trusted), traffic carries a datacenter ASN and reinforces bot classification — and geolocation points at your proxy's region.

**Fix:**

1. Set `X-Forwarded-For` to the original visitor IP before forwarding to Rybbit.
2. Enable the **First-Party Proxy** toggle in **Site Settings → Privacy & Security** so Rybbit trusts the forwarded IP unconditionally.

<Callout type="warn">
Use `X-Forwarded-For`, not `X-Real-IP`. On Rybbit Cloud, `X-Real-IP` never reaches the backend (our edge network consumes it), and AWS CloudFront cannot send it at all.
</Callout>
...
const proxyHeaders = new Headers(request.headers); // preserves all browser headers
proxyHeaders.set('X-Forwarded-For', clientIp);
```

**AWS CloudFront:**

- Set `X-Forwarded-For` to the viewer IP from a CloudFront Function/Lambda@Edge
- Configure the origin request policy to forward all viewer headers
...
- [ ] Client IP is forwarded in `X-Forwarded-For` (not just `X-Real-IP` — that header doesn't survive Rybbit Cloud's edge)
- [ ] **First-Party Proxy** toggle is enabled in Site Settings → Privacy & Security
- [ ] Full browser header set is forwarded (`Accept`, `Accept-Language`, `Accept-Encoding`, `Sec-Fetch-*`) — not just `User-Agent`
