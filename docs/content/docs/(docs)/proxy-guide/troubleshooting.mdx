...
<Callout type="warn">
**Critical**: The `X-Forwarded-For` (or `X-Real-IP`) header must contain the original client IP for accurate geolocation and user identification. Rybbit reads both before the connecting edge IP. On AWS CloudFront use `X-Forwarded-For` — it cannot forward `X-Real-IP`.
</Callout>
...
- Events do not appear in the normal analytics dashboard
- Bot logs or older responses say the event was not tracked because a bot was detected
- The bot reason mentions `bot asn`
- Geolocation points to your CDN or proxy region, such as a Cloudflare Worker or AWS CloudFront edge location

**Cause:** Rybbit is seeing the proxy IP instead of the original visitor IP. This is common when a first-party proxy is hosted on CDN or cloud infrastructure. Those networks use hosting/datacenter ASNs, so traffic can look like bot infrastructure if the visitor IP is not passed in a trusted way.

Set `X-Forwarded-For` (or `X-Real-IP`) to the original visitor IP before forwarding the request to Rybbit. Rybbit reads both before the connecting edge IP. `X-Real-IP` wins if both are present, but `X-Forwarded-For` is fully supported — use it on AWS CloudFront, which cannot forward `X-Real-IP`.
