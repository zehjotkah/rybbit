...
- For CDN or edge proxies, forward the visitor IP in `X-Forwarded-For` or `X-Real-IP`
...
| `X-Real-IP` or `X-Forwarded-For` | IP address for geolocation | Yes |
| `Referer` | Referrer tracking | Recommended |
| `Accept-Language` | Language detection | Optional |

<Callout type="warn">
**IP Forwarding**: Rybbit uses IP addresses for geolocation (country/city) and creating anonymous user identifiers (hashed IP + User Agent). If you don't forward the `X-Real-IP` or `X-Forwarded-For` header, all traffic will appear to come from your proxy server's IP.
</Callout>

### Forwarded IP Headers

If your proxy runs on infrastructure such as Cloudflare Workers, AWS CloudFront, Fastly, or another edge/CDN provider, make sure the proxy sets `X-Forwarded-For` (or `X-Real-IP`) to the original visitor IP. Rybbit reads both **before** the connecting edge IP, so this prevents traffic from being geolocated to the proxy's exit node. `X-Real-IP` takes precedence if both are present, but `X-Forwarded-For` is fully supported and is the right choice on platforms that can't set `X-Real-IP` — notably AWS CloudFront, where `X-Real-IP` is on the [origin custom-header denylist](https://docs.aws.amazon.com/AmazonCloudFront/latest/DeveloperGuide/add-origin-custom-headers.html#add-origin-custom-headers-denylist).
...
4. **Incorrect geolocation**: Verify `X-Forwarded-For` or `X-Real-IP` is set to the visitor IP
