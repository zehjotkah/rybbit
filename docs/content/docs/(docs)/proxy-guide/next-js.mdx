...
<Callout type="info">
Enable **First-Party Proxy** in your Rybbit **Site Settings → Privacy & Security**. Rewrites on this platform automatically forward the browser's headers and set `X-Forwarded-For` to the visitor IP — the toggle tells Rybbit to trust that forwarded IP directly.
</Callout>
...
The Rybbit server sets cache headers on its scripts, which Next.js respects:
- Scripts (`.js` files): Cached for 1 hour (`script.js`) to 1 day (`replay.js`, `metrics.js`)
- Configuration: Not cached by the server (you may cache it for ~5 minutes at your proxy if desired)
