...
Per-include attributes like `data-site-id` require TYPO3 12.1+ (Feature #91499).
On older TYPO3 versions `includeJS` cannot emit `data-*` attributes, so you'll
need a different install method (for example, adding the full `<script>` tag via
`page.headerData`).
