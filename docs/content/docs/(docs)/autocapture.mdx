...
**Tracked properties:**
- `text` - The button's text content (falls back to `aria-label`, then `value` for input buttons, then `title`)
...
- `ariaLabel` - The form's `aria-label` attribute (if present)
...
Triggered when a user changes the value of an input, select, or textarea field. Disabled inputs are automatically excluded from tracking.

**Tracked properties:**
- `element` - The element type (`input`, `select`, or `textarea`)
- `inputType` - For inputs: the type attribute (e.g., `text`, `email`, `checkbox`)
- `inputName` - The field's `name`, `id`, `aria-label`, or `placeholder` attribute (first available)
- `formId` - The parent form's `id` (if within a form)
- `formName` - The parent form's `name` (if within a form)
...
**Privacy note:** Hidden inputs, password fields, and disabled inputs are automatically excluded from tracking.
