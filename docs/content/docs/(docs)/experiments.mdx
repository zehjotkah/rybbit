...
| **Exposure** | When your code reads the flag with `rybbit.flag()`, Rybbit records an exposure for that variant. The number of distinct sessions exposed to a variant is the denominator for its conversion rate. |
...
| **Conversion rate** | Conversions divided by the number of exposed sessions (the **Sessions** figure) for that variant. |
