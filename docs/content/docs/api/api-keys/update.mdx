...
<TypeTable
  type={{
    ...
    rateLimitEnabled: {
      description: 'Legacy per-key rate limit toggle. Has no effect — [rate limits](/docs/api/getting-started#rate-limits-cloud-only) are enforced per credential owner and follow that owner\'s plan',
      type: 'boolean',
      required: false,
    },
  }}
