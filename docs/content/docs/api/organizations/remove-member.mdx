...
<Callout type="warn">This endpoint uses **POST**, not HTTP DELETE. You cannot remove the only remaining owner of an organization.</Callout>
...
<TypeTable
  type={{
    memberIdOrEmail: {
      description: "The member's ID or email address",
      type: 'string',
      required: true,
    },
    organizationId: {
      description: 'The organization to remove from. Defaults to your active organization.',
      type: 'string',
      required: false,
    },
  }}
...
<TypeTable
  type={{
    ...
    organizationId: {
      description: 'The organization the member belonged to',
      type: 'string',
    },
    role: {
...
curl -X POST "https://app.rybbit.io/api/auth/organization/remove-member" \
  ...
  -d '{"memberIdOrEmail": "teammate@example.com"}'
```
  </Tab>
