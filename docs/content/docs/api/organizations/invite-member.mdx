---
title: Invite Member
description: Send an organization invitation to a user by email.
full: true
method: POST
---
...
Sends an invitation email inviting a user to join an organization with a given role. (To add an existing user directly without an email invite, see Add Member.)
...
<TypeTable
  type={{
    ...
    role: {
      description: 'Role(s) to assign. One of `owner`, `admin`, `member` (or an array of roles).',
      type: 'string | string[]',
      required: true,
    },
    organizationId: {
      description: 'The organization to invite into. Defaults to your active organization.',
      type: 'string',
      required: false,
    },
    resend: {
      description: 'Resend the invitation if the user was already invited.',
      type: 'boolean',
      required: false,
    },
    teamId: {
      description: 'Team ID(s) to add the invited user to.',
      type: 'string | string[]',
      required: false,
    },
    hasRestrictedSiteAccess: {
      description: 'Rybbit extension: restrict the invited member to specific sites. Only valid when role is `member`.',
      type: 'boolean',
      required: false,
    },
    siteIds: {
      description: 'Rybbit extension: the site IDs the member may access when `hasRestrictedSiteAccess` is true.',
      type: 'number[]',
      required: false,
    },
  }}
...
<Callout>`hasRestrictedSiteAccess` and `siteIds` are Rybbit-specific extensions to the invitation. Site-access restrictions can only be applied to invitations with the `member` role.</Callout>
...
<TypeTable
  type={{
    ...
    hasRestrictedSiteAccess: {
      description: 'Whether the member will be restricted to specific sites',
      type: 'boolean',
    },
    siteIds: {
      description: 'Site IDs the member will be allowed to access',
      type: 'number[]',
    },
  }}
/>
